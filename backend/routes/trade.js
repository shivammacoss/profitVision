...
router.post('/open', async (req, res) => {
  try {
    ...
    if (masterTrader) {
      // Log master trader info (no trade limit)
      const existingOpenTrades = await Trade.find({
        tradingAccountId,
        status: 'OPEN'
      })
      console.log(`[Trade] Master ${masterTrader._id} has ${existingOpenTrades.length} open trades - no limit enforced`)
    }
    ...
    setImmediate(async () => {
      try {
        ...
        if (user && user.bookType === 'A') {
          ...
          const result = await lpService.pushTradeToCorecen(trade, user)
          if (!result.success) {
            console.error('[Trade] Failed to push A-Book trade to Corecen:', result.error || result.message)
          }
        }
      } catch (lpError) {
        console.error('[Trade] Error pushing A-Book trade to Corecen:', lpError)
      }
    })
  } catch (error) {
...
router.post('/close', async (req, res) => {
  try {
    ...
    // Get trade first to check symbol and fetch price if needed
    ...
    // If no price provided, try to fetch from LP cache
    if (!bid || !ask || parseFloat(bid) <= 0 || parseFloat(ask) <= 0 || isNaN(parseFloat(bid)) || isNaN(parseFloat(ask))) {
      try {
        // Try to get price from LP cache
        const { getLpPrice } = await import('./lpIntegration.js')
        const lpPrice = getLpPrice(tradeToClose.symbol)
        
        if (lpPrice && lpPrice.bid && lpPrice.ask) {
          bid = lpPrice.bid
          ask = lpPrice.ask
          console.log(`[Trade Close] Using LP cache price for ${tradeToClose.symbol}: bid=${bid}, ask=${ask}`)
        }
      } catch (lpErr) {
        console.log('[Trade Close] Could not fetch LP price:', lpErr.message)
      }
    }

    // Final check - if still no valid price, reject
    if (!bid || !ask || parseFloat(bid) <= 0 || parseFloat(ask) <= 0 || isNaN(parseFloat(bid)) || isNaN(parseFloat(ask))) {
      return res.status(400).json({ 
        success: false, 
    ...
    setImmediate(async () => {
      try {
        ...
        if (user && user.bookType === 'A') {
          ...
          const closeResult = await lpService.closeTradeOnCorecen(result.trade)
          if (!closeResult.success) {
            console.error('[Trade] Failed to close A-Book trade on Corecen:', closeResult.error || closeResult.message)
          }
        }
      } catch (lpError) {
        console.error('[Trade] Error closing A-Book trade on Corecen:', lpError)
      }
    })
