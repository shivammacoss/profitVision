...
class CreditRefillService {
  ...
  async _handleLoss(params) {
    ...
    // This maintains the minimum deposit requirement set by master trader
    if (creditAfter < minimumCredit) {
      const deficit = minimumCredit - creditAfter
      const wallet = await Wallet.findOne({ userId })
      const walletBalance = wallet?.balance || 0

      console.log(`[CreditRefill] ⚠️ CREDIT BELOW MINIMUM!`)
      console.log(`[CreditRefill] Credit: $${creditAfter.toFixed(2)}, Minimum Required: $${minimumCredit}`)
      console.log(`[CreditRefill] Deficit: $${deficit.toFixed(2)}, Wallet Balance: $${walletBalance.toFixed(2)}`)
      ...
      if (walletBalance > 0) {
        ...
        console.log(`[CreditRefill] ✅ AUTO-REFILL SUCCESS!`)
        console.log(`[CreditRefill] Transferred $${walletRefillAmount.toFixed(2)} from Main Wallet to Copy Account`)
        console.log(`[CreditRefill] New Credit Balance: $${creditAfter.toFixed(2)}`)
